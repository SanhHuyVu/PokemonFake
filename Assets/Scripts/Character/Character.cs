...
public class Character : MonoBehaviour
{
    ...
    public IEnumerator Move(Vector2 moveVec, Action OnMoveOver = null)
    {
        ...
        // stop the coroutine if targetpos is not walkable
        if (IsPathClear(targetPos) == false)
            yield break;
    ...
    {
        var diff = targetPos - transform.position; // get the lenght  
        var dir = diff.normalized; // get the deirection (vector)
    ...
    public void LookTowards(Vector3 targetPos)
    {
        ...
        if (xdiff == 0 || ydiff == 0)
        {
            ...
            Debug.Log("I CANT LOOK DIAGONALLY!");
    }
